from entity_dict import entity_dict_BEL, entity_dict_FRA
import pymongo
...
for k, v in chain(entity_dict_BEL.items(), entity_dict_FRA.items()):
...
for post in db.BELFRA_SEMI.find():
	...
	for i in entity_dict_final:
		for j in entity_dict_final[i]:
			if(text in j):
				db.BELFRA_SEMI.update({"_id": _id}, {"$set": {"entity_name": i}}, upsert=True)
	counter += 1
	print counter
